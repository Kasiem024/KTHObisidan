---
tags: [begrepp, HI1031, databaser, programmering, KTH, year2026]
created: 2026-08-24
updated: 2026-08-24
description: "Begrepp: indirekt kommunikation (HI1031)."
---
# Indirekt kommunikation

## Definition

Kommunikation ==via en mellanhand==, där sändare och mottagare inte pratar direkt (och ofta inte känner varandra). Ger *rumslig* frikoppling (behöver inte veta vem) och *tidsmässig* frikoppling (behöver inte finnas samtidigt). Exempel: gruppkommunikation, publish-subscribe, meddelandeköer, tuple spaces.

## Kopplat till

- [[Middleware]]
- [[Replikering]]

## Flashcards

**Indirekt kommunikation**;;Kommunikation ==via en mellanhand==, där sändare och mottagare inte pratar direkt med varandra.

Vilka två former av frikoppling ger indirekt kommunikation? (2)
||
- **Rumslig** – behöver inte veta *vem* som tar emot
- **Tidsmässig** – behöver inte finnas till *samtidigt*
